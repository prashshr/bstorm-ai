...
Usage:
    python testing/scripts/run-tests.py               # Run backend tests
    python testing/scripts/run-tests.py --coverage     # Run with coverage report
    python testing/scripts/run-tests.py --infra        # Include infrastructure checks
    python testing/scripts/run-tests.py --quick        # Run only core tests (skip slow)
    python testing/scripts/run-tests.py --view         # View most recent report

The script:
  1.  Captures the current git version (tag + commit hash + date)
  2.  Runs pytest against testing/backend/{unit,integration}/ with JUnit XML output
  3.  Parses results and generates a professional markdown report
  4.  Saves report as: testing/reports/test-report_{VERSION}_{YYYYMMDD}_{HHMM}.md
"""
...
TEST_BACKEND_DIR = TESTING_DIR / "backend"
...
def run_pytest(extra_args: Optional[List[str]] = None) -> Tuple[int, str, str]:
    ...
    test_paths = [str(TEST_BACKEND_DIR / "unit"), str(TEST_BACKEND_DIR / "integration")]
    cmd = [
        python, "-m", "pytest",
        *test_paths,
        "-c", str(TEST_BACKEND_DIR / "pyproject.toml"),
        "-v",
        "--tb=short",
...
def run_coverage() -> str:
    ...
    rc, stdout, stderr = run_command(cmd, cwd=PROJECT_ROOT, timeout=600)
    return stdout + "\n" + stderr
